"""
LangChain callback adapter for AgentBlackBox.

Automatically records tool calls, chain events, and agent actions
as AGENT_THOUGHT events in the AgentBlackBox audit trail.

Install:
    pip install 'agentblackbox[langchain]'
...
Usage:
    ...
    with abb.session(agent_id="my-agent") as s:
        handler = AgentBlackBoxLangChain(s)
...
try:
    ...
    raise ImportError(
        "LangChain is not installed. Install with: pip install 'agentblackbox[langchain]'"
    )

from typing import Any, Union


class AgentBlackBoxLangChain(BaseCallbackHandler):
    """
    LangChain BaseCallbackHandler that records events to AgentBlackBox.
    ...
    def __init__(self, session: Any):
        """
        Args:
            session: An agentblackbox.Session instance
        """
        super().__init__()
