...
Quick start::
    ...
    import chromadb
    from agentblackbox.memory import wrap_chroma, ScanConfig
...
Environment variables read by :func:`~agentblackbox.config.default_config`:
    ABB_BACKEND_URL, ABB_API_KEY, ABB_AGENT_ID, ABB_SESSION_ID
"""
...
class ScanConfig:
    ...
    backend_url: str = ""
    """AgentBlackBox backend URL for event reporting (optional)."""
...
def _get_config(config: Optional[ScanConfig]) -> ScanConfig:
    if config is not None:
        return config
    try:
        from agentblackbox.config import default_config  # noqa: PLC0415
        return default_config()
    except Exception:
        return ScanConfig()


def wrap_chroma(client: Any, *, config: Optional[ScanConfig] = None) -> Any:
    """
    ...
    config : ScanConfig, optional
        Scanner configuration.  Defaults to :func:`~agentblackbox.config.default_config`.
    ...
    """
    from agentblackbox.memory.wrappers.chroma import wrap as _wrap  # noqa: PLC0415
    return _wrap(client, config=_get_config(config))


def wrap_langchain_memory(memory: Any, *, config: Optional[ScanConfig] = None) -> Any:
    """
    ...
    """
    from agentblackbox.memory.wrappers.langchain import wrap as _wrap  # noqa: PLC0415
    return _wrap(memory, config=_get_config(config))


def wrap_pinecone(index: Any, *, config: Optional[ScanConfig] = None) -> Any:
    """
    ...
    """
    from agentblackbox.memory.wrappers.pinecone import wrap as _wrap  # noqa: PLC0415
    return _wrap(index, config=_get_config(config))


def wrap_weaviate(client: Any, *, config: Optional[ScanConfig] = None) -> Any:
    """
    ...
    """
    from agentblackbox.memory.wrappers.weaviate import wrap as _wrap  # noqa: PLC0415
    return _wrap(client, config=_get_config(config))


def wrap_callable(fn: Callable, *, config: Optional[ScanConfig] = None) -> Callable:
    """
    ...
    Use this for unsupported vector stores::
        ...
        from agentblackbox.memory import wrap_callable
    ...
    """
    from agentblackbox.memory.wrappers.callable import wrap as _wrap  # noqa: PLC0415
